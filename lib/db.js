...
export async function saveVote(
  ...
  try {
    const result = await sql`
      ...
      VALUES (${voterEmail}, ${candidateId}, ${JSON.stringify(
      aspectScores
    )}, ${isAbstained}, ${isPartial})
...
export async function getVotingStatistics() {
  try {
    ...
    allVotes.forEach((vote) => {
      ...
      if (
        !latestVotes[key] ||
        new Date(vote.updated_at) > new Date(latestVotes[key].updated_at)
      ) {
      ...
      if (vote.is_abstained) {
        ...
        if (aspectScores && typeof aspectScores === "object") {
          ...
          if (validAspectScores.length > 0) {
            ...
            candidateStats[vote.candidate_id].scores.push(weightedScore);
          }
        }
      }
    });

    // Calculate average scores and apply weights
    const candidateStatsResult = Object.values(candidateStats)
      .map((stat) => {
        // Calculate average score if there are valid scores
        let averageScore = null;
        if (stat.scores.length > 0) {
          // Calculate the average of all valid scores
          const sum = stat.scores.reduce((a, b) => a + b, 0);
          averageScore = sum / stat.scores.length;

          // Ensure the score is within 0-100 range
          averageScore = Math.min(Math.max(averageScore, 0), 100);
        }
